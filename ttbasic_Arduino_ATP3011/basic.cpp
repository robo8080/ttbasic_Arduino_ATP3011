...
AquesTalk talk;  //インスタンス定義
void io_init()
{
    if (talk.IsActive()) {
        Serial.println("ATP3011 OK\r\n");
    } else {
        Serial.println("ATP3011 NG\r\n");
    }
//    const char* msg = "setsuzo_kushima'_shita.";
//    talk.Synthe(msg);
}
...
//-------------------------------------------------------------
void talknum(short value, short d){
    unsigned char i;
    unsigned char sign;
    char buff[130];
    ...
    if(sign) {
        snprintf(buff, sizeof(buff), "mainasu<ALPHA VAL VAL=%s>", &lbuf[i]);
    } else {
        snprintf(buff, sizeof(buff), "<ALPHA VAL VAL=%s>", &lbuf[i]);
    }
    talk.Synthe(buff);
}
    ...
    while(1){
        ...
        default:
            ...
            if(len > 0) {
                ...
                if(value == 0){
                    snprintf(buff, sizeof(buff), "ze'ro");
                } else if(value < 0){
                    value = -value;
                    snprintf(buff, sizeof(buff), "mainasu<NUMK VAL=%d>", value);
                } else {
                    snprintf(buff, sizeof(buff), "<NUMK VAL=%d>", value);
                }
                talk.Synthe(buff);
            }
            break;
...
void iktalk()
{
    ...
    if(value < 0){
        value = -value;
        snprintf(buff2, sizeof(buff2), "mainasu<NUMK VAL=%d COUNTER=%s>.", value, buff1);
    } else {
        snprintf(buff2, sizeof(buff2), "<NUMK VAL=%d COUNTER=%s>.", value, buff1);
    }
    talk.Synthe(buff2);
 }
//-------------------------------------------------------------
